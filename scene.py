...
def build_vault_scenes():
    """Builds the vault section and links the scenes together.
    Technique: sequence unpacking (#6)
    """
    vault_door = Scene(
        ...
        ["Pick the lock", "Look for a key nearby", "Force it open"]
    )
    pick_lock = Scene(
    ...
    vault_interior = LootScene(
        ...
        ["Open the chest", "Take the documents", "Take everything you can carry"],
        loot="gold"
    )
    ...
    return vault_door, vault_interior


def build_escape_scenes(vault_interior):
    """Builds the escape routes and connects them to the vault.
    Technique: use of a key function with sorted() (#9)
    """
    escape_light = Scene(
        ...
        ["Back through the drainage tunnel", "Walk out the gate", "Over the wall"]
    )
    escape_heavy = Scene(
    ...
    end_all = Scene(
        "The drainage tunnel is tighter than you remember. The bag catches on every edge, every rusted bolt. You drag it through standing water that soaks through to your knees.\n\nBut you come out the other side with everything. Gold in the bag. Documents in your shirt. Wet, stinking, and rich.\n\n--- ESCAPED WITH EVERYTHING ---",
        ["Start over"]
    )
    fail_gate_heavy = Scene(
    ...
    fail_wall_heavy = Scene(
        "You hook your fingers into the first gap in the stone and pull. The bag swings. The gold inside shifts and the strap cuts into your shoulder. You get one arm over the top before the weight drags you back down.\n\nThe bag splits. Coins scatter across the flagstones.\n\nThe guards find you sitting in a pile of your own loot.\n\n--- CAUGHT ---",
        ["Start over"]
    )
    endings = [end_gold, end_docs, end_all, fail_gate_heavy, fail_wall_heavy]
    endings_sorted = sorted(endings, key=lambda s: len(s.prose))
    escape_light.add_next(1, end_docs)
    escape_light.add_next(2, end_gold)
    escape_light.add_next(3, end_docs)
    escape_heavy.add_next(1, end_all)
    escape_heavy.add_next(2, fail_gate_heavy)
    escape_heavy.add_next(3, fail_wall_heavy)
    vault_interior.add_next(1, escape_heavy)
    vault_interior.add_next(2, escape_light)
    vault_interior.add_next(3, escape_heavy)
    for scene in [end_gold, end_docs, end_all, fail_gate_heavy, fail_wall_heavy]:
        scene.add_next(1, approach if False else None)
    return vault_interior
...
basement_hall = Scene(
    ...
    ["Follow the glow", "Check the side passages first"]
)
...
shove_path = Scene(
    ...
    ["Duck into the doorway", "Keep running toward the inner keep"]
)
shove_doorway = Scene(
...
grate = Scene(
    ...
    ["Go left, toward the noise", "Go right, toward the silence"]
)
wall_climb = Scene(
    ...
    ["Freeze and wait", "Drop back down and try the grate"]
)
wall_freeze = Scene(
...
river = Scene(
    ...
    ["Climb through the broken grate", "Scale the wall"]
)
...
nightfall = Scene(
    ...
    ["Approach the gate", "Circle to the river side"]
)
nightfall_gate = Scene(
    ...
    ["\"Wine for the captain's table. Got held up on the road.\"", "\"Message from the magistrate. It's urgent.\""]
)
...
bluff = Scene(
    ...
    ["\"Wine for the captain's table.\"", "\"Message from the magistrate. Urgent.\"", "Shove past him and run"]
)
...
approach = Scene(
    ...
    ["Walk up to the gate and bluff your way in", "Circle around to the river side", "Wait until nightfall"]
)

# WIRING

vault_door, vault_interior = build_vault_scenes()
build_escape_scenes(vault_interior)

# end states loop back to approach
...
fail_force_vault = Scene(
    "The third hit echoes down the corridor like a bell.\n\n--- CAUGHT ---",
    ["Start over"]
)
